...
class TestViewTrainers(unittest.TestCase):
    ...
    def setUp(self):
        self.trainer = Trainer('Anne', 'T001', 'anne@lms.com', "Process Change Management, Aftersales IT Support, Software Development", "8 years experience in IT operation", "IS111, IS212, IS213, IS216")
    
    def tearDown(self):
        self.trainer = None

    # Check if trainer exists >> ID, Name, list of couress taught, skills, experience
...
class TestViewEligibleCourses(unittest.TestCase):
    ...
    def setUp(self):
        self.learner = Learner('Alivia', 'L001', 'alivia@lms.com', "IS111, IS213, IS215")
        self.course = Course('IS212', 'Software Project Management', '...', "IS111, IS213", 2, "G1, G2]", 'Project Management')
        self.class1 = Classes("G1", "IS212", 20, "T001", datetime(2021, 10, 1), datetime(2022, 11, 30))
        self.trainer = Trainer('Anne', 'T001', 'anne@lms.com', "Process Change Management, Aftersales IT Support, Software Development", "8 years experience in IT operation", "IS111, IS212, IS213, IS216")
    ...
    def testEligibility(self):
        ...
        self.assertEqual(self.learner.courseEligibility(prerequisite), self.learner.coursesTaken)
        self.assertRaises(Exception, self.learner.courseEligibility, 'IS200, IS111')
...
class TestApplication(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.class1 = Classes("G1", "IS212", 20, "T001", datetime(2021, 10, 1), datetime(2022, 11, 30))
        self.trainer = Trainer('Anne', 'T001', 'anne@lms.com', "Process Change Management, Aftersales IT Support, Software Development", "8 years experience in IT operation", "IS111, IS212, IS213, IS216")
        self.application1 = Application(1, "L001", "G1", "IS212", "Draft", datetime(2021, 8, 1), datetime(2021, 9, 1), "admin001")
    ...
    def test_existApplication(self):
        self.assertEqual(self.application1.json(), {
            ...
            'regStartDate': datetime(2021, 8, 1),
            'regEndDate': datetime(2021, 9, 1),
            'adminID': "admin001"}
            )
    ...
    # check the change in application status works
    def test_changeStatus(self):
        self.assertEqual(self.application1.applicationStatus, "Draft")
        changeStatus = self.application1.changeStatus("Processing")
        self.assertEqual(changeStatus, "Processing")
        changeStatus = self.application1.changeStatus("Withdrawn")
        self.assertEqual(changeStatus, "Withdrawn")
    ...
    def test_checkEnrolmentPeriod(self):
        self.assertRaises(Exception, self.application1.checkEnrolmentPeriod, datetime.now())
